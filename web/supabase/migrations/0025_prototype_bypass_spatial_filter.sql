-- =============================================================================
-- 0025 — PROTOTYPE: customer_find_gigs bypass spatial filter
-- =============================================================================
-- In testing, the customer GPS coordinates and the worker's seeded coordinates
-- may not match the same city. Rather than fighting GPS noise, we override the
-- RPC to return ALL active gigs in the matching service, ignoring distance.
-- The 'distance_km' field will still be computed and returned for display.
-- =============================================================================
...
begin
  ...
  -- PROTOTYPE: spatial filter completely removed. Return all ACTIVE gigs for the
  -- service from ACTIVE workers. Distance is still computed for display.
  return query
  ...
  where g.service_id = p_service_id
    and g.status     = 'ACTIVE'
    and w.status     = 'ACTIVE'
    -- PROTOTYPE: no KYC, no distance filter
  order by
