-- worker_eligibility: reasons that don't contradict each other.
--
-- Found in device testing: a worker whose identity check was never completed
-- saw both "Your documents are being reviewed" (from the VERIFICATION_PENDING
-- account status) and "Complete identity verification" (from the KYC gate).
-- The generic status line is now only used for statuses the specific gates
-- below don't already explain, and the KYC and background-check lines reflect
-- the actual state of each check (not started, under review, or rejected).
--
-- Also fixes the service-area and trade actions, which pointed at routes that
-- do not exist in the worker app ('profile/service-area', 'profile/trade').
