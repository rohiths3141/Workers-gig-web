...
export default async function WorkerDetailPage({
  ...
  // New workers sit in REGISTERED / VERIFICATION_PENDING until an admin
  // activates them; approving checks alone never changes the account status.
  const awaitingActivation =
    worker.status === WorkerStatus.REGISTERED || worker.status === WorkerStatus.VERIFICATION_PENDING;
  const checksApproved = worker.is_kyc_verified && worker.is_background_verified;
              ...
              <>
                {awaitingActivation && (
                  <ActionButton
                    endpoint={apiRoutes.adminWorkerStatus(worker.id)}
                    payload={{ status: WorkerStatus.ACTIVE }}
                    label="Activate worker"
                    variant="success"
                    tone="success"
                    confirmTitle="Activate this worker?"
                    confirmDescription={
                      checksApproved
                        ? 'Identity and background checks are approved. They can go available and customers can find their live services.'
                        : 'Identity or background check is not approved yet. They will not be able to go available until both are approved.'
                    }
                    confirmLabel="Activate"
                    requireReason
                    reasonLabel="Why is this account being activated?"
                    successMessage="Worker activated"
                  />
                )}
                <ActionButton
                  endpoint={apiRoutes.adminWorkerStatus(worker.id)}
